...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$term_query, {
    output$query_result_agg_by_index <- DT::renderDataTable(
      DT::datatable(
        get_query_result_agg_by_index(input$term_query),
        filter = "top",
        selection = "single",
        rownames = F,
        options = list(
  ...
  observeEvent(input$query_result_agg_by_index_row_last_clicked, {
    ...
    nomenclature <- get_query_result_agg_by_index(input$term_query)$key[index_tmp]
    output$query_result <- DT::renderDataTable(
      DT::datatable(
        get_query_result(input$term_query,nomenclature),
        filter = "top",
        selection = "single",
        rownames = F,
        options = list(
